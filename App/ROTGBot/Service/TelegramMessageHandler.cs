...
namespace ROTGBot.Service
{
    public class TelegramMessageHandler : ITelegramMessageHandler
    {
            ...
            CancellationToken token)
        {
            ...
            return data switch
            {
                ...
                "UserSearchByNameChoise" => await SendWithCheckRights(user, chatId.Value, RoleEnum.administrator,
                                        (chId, userNews, tk) => SendUserSearchByNameChoiseHandle(chId, user.Id, userNews, tk), token),
        ...
        private async Task UserSearchByNameHandle(Guid userId, long chatId, News? userNews, CancellationToken token)
        {
            if (userNews != null)
            {
                await SendUserSearchByNameApprove(userId, chatId, userNews, token);
            }
            else
            {
        ...
        private async Task UserSearchByNameMessageNotFound(long chatId, CancellationToken token)
        {
            throw new NotImplementedException();
        }
        ...
        private async Task SendUserInfoByNewsNumberHandle(Guid userId, long chatId, News? userNews, CancellationToken token)
        {
            if (userNews != null)
            {
                await SendUserInfoByNewsNumberAccepted(userId, chatId, userNews, token);
            }
            else
            {
                await SendUserInfoByNewsNumberMessageNotFound(chatId, token);
            }
        }
        ...
        private async Task SendUserInfoByNewsNumberChoiseHandle(long chatId, Contract.Model.User user, News? userNews, CancellationToken token)
        {
            ...
            {
                await SendUserInfoByNewsNumberChoise(chatId, user.Id, token);
            }
        }
        ...
        private async Task SendUserInfoByNewsNumberAccepted(Guid userId, long chatId, News userNews, CancellationToken token)
        {
            ...
            if(searchNews == null)
            {
                await client.SendMessageAsync(chatId, "Новость по номеру не найдена", token);
                return;
            }
